  ...
  return (
    <div className="mx-auto max-w-6xl px-6 py-8">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="font-headings text-xl font-semibold text-foreground-primary">模板管理</h1>
          <p className="mt-0.5 text-xs text-foreground-muted">
            设计师维护模板，发布后 BD 可基于模板创建报告。
          </p>
        </div>
      ...
      {loading ? (
        ...
        <p className="mt-8 text-sm text-foreground-muted">还没有模板，新建一个开始吧。</p>
      ) : filtered.length === 0 ? (
        <p className="mt-8 text-sm text-foreground-muted">没有符合筛选条件的模板。</p>
      ) : (
