...
export function CreatorWorksTable({ data }: { data: CreatorWorksTableData }) {
  ...
  if (variant === 'compact') {
    // 纯文本紧凑行，无图片。
    return (
      <div className="flex h-full w-full flex-col skin-gap-xs skin-card skin-pad-sm">
        {(title || subtitle) && (
        ...
        <div className="flex flex-1 flex-col overflow-auto">
          <div className="flex border-b border-border-default pb-1 text-[10px] skin-fw-body text-foreground-muted">
  ...
  if (variant === 'cards') {
    ...
    return (
      <div className="flex h-full w-full flex-col gap-1.5 skin-card skin-pad-sm">
        ...
        <div className="grid flex-1 grid-cols-2 skin-gap-sm overflow-auto">
          {items.map((it, i) => (
            <div key={i} className="flex flex-col skin-gap-xs rounded-lg border border-border-subtle p-2">
              ...
              <div className="line-clamp-1 text-xs skin-fw-body text-foreground-primary">{it.name}</div>
  ...
  if (variant === 'insight') {
    ...
    return (
      <div className="flex h-full w-full flex-col skin-gap-xs skin-card skin-pad-sm">
        ...
        <div className="flex flex-1 flex-col skin-gap-xs overflow-auto">
          <div className="flex items-center skin-gap-sm border-b border-border-default pb-1.5">
            <span className="w-10 flex-none" />
            <span className="min-w-0 flex-1 text-[10px] skin-fw-body uppercase tracking-wide text-foreground-muted">
              {headers[1] ?? 'Title'}
            </span>
            {numHeaders.map((h, i) => (
              <span key={i} className="w-14 flex-none text-right text-[10px] skin-fw-body uppercase tracking-wide text-foreground-muted">
                {h}
              </span>
            ))}
          </div>
            ...
            return (
              <div key={i} className="rounded-lg border border-border-subtle p-1.5">
                ...
                <div className="flex items-center skin-gap-sm">
                  ...
                  <div className="min-w-0 flex-1 truncate text-xs skin-fw-body text-foreground-primary">{it.name}</div>
                  <span className="w-14 flex-none text-right font-data text-xs skin-fw-heading text-foreground-primary">{it.play}</span>
                  <span className="w-14 flex-none text-right font-data text-xs text-foreground-secondary">{it.like}</span>
                  <span className="w-14 flex-none text-right font-data text-xs text-foreground-secondary">{it.completion}</span>
                </div>
                {/* 展开行：性别条 + Top3 城市 + sparkline */}
                {hasInsight && (
                  <div className="mt-1.5 grid grid-cols-3 skin-gap-sm border-t border-border-subtle pt-1.5">
                    {/* 性别 */}
                    ...
                    {ins.topCities && ins.topCities.length > 0 && (
                      <div className="flex flex-col gap-0.5">
                        ...
                        {ins.topCities.slice(0, 3).map((c, ci) => (
                          <div key={ci} className="flex items-center skin-gap-xs">
  ...
  return (
    <div className="flex h-full w-full flex-col skin-gap-xs skin-card skin-pad-sm">
      ...
      <div className="flex flex-1 flex-col overflow-auto">
        ...
        {items.map((it, i) => (
          <div key={i} className="flex items-center skin-gap-sm border-b border-border-subtle py-1.5 last:border-b-0">
